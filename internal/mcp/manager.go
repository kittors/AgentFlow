...
func (m *Manager) installIntoVSCodeMCP(path string, spec BuiltinSpec) error {
	settings, err := readJSONMap(path)
	if err != nil {
		return err
	}
	...
	if existing, ok := settings["servers"]; ok {
		mapped, ok := existing.(map[string]any)
		if !ok {
			return fmt.Errorf("%s: servers must be an object", filepath.Base(path))
		}
		servers = mapped
	}
...
func (m *Manager) removeFromVSCodeMCP(path, name string) error {
	settings, err := readJSONMap(path)
	if err != nil {
		return err
	}

	existing, ok := settings["servers"]
	if !ok {
		return nil
	}
...
func (m *Manager) listVSCodeMCP(path string) (map[string]any, error) {
	settings, err := readJSONMap(path)
	if err != nil {
		return nil, err
	}
	...
	servers, ok := existing.(map[string]any)
	if !ok {
		return nil, fmt.Errorf("%s: servers must be an object", filepath.Base(path))
	}
	return servers, nil
}
